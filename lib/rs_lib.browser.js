...
// source-hash: 2fc68c5f8ab8e921db6817c8fa319db0ddf21117
let wasm;
 ...
 * @param {number} x
 * @param {number} y
 * @param {number} face_width
 * @param {number} face_height
 * @returns {number}
 */
export function load_image(data, x, y, face_width, face_height) {
  const ptr0 = passArray8ToWasm0(data, wasm.__wbindgen_malloc);
  const len0 = WASM_VECTOR_LEN;
  const ret = wasm.load_image(ptr0, len0, x, y, face_width, face_height);
  return ret >>> 0;
}
...
const wasm_url = new URL("http://localhost:8000/rs_lib_bg.wasm");
 ...
 *   exports: { add: typeof add; load_image: typeof load_image; init_panic_hook: typeof init_panic_hook }
 * }>}
 */
...
function getWasmInstanceExports() {
  return { add, load_image, init_panic_hook };
}
